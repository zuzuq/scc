...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/zuzuq/izinvps/main/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/zuzuq/izinvps/main/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/zuzuq/izinvps/main/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/zuzuq/izinvps/main/ip | grep $MYIP | awk '{print $2}')
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "victortjandra062@gmail.com" &> /dev/null
    git config --global user.name "zuzuq" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M ipuk &> /dev/null
    git remote add origin https://github.com/zuzuq/backup
    git push -f https://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@github.com/zuzuq/backup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/zuzuq/backup/main/$NameUser/$NameUser.zip"
sleep 1
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • ZUZUQ •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo
